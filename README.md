<p align="center">
  <a href="https://github.com/delta10/error-pages#readme">
    <picture>
      <source media="(prefers-color-scheme: dark)" srcset="https://socialify.git.ci/delta10/error-pages/image?description=1&font=Raleway&forks=1&issues=1&logo=https%3A%2F%2Fwww.delta10.nl%2F_next%2Fstatic%2Fmedia%2Flogo.e75acb6d.svg&owner=1&pulls=1&pattern=Solid&stargazers=1&theme=Dark">
      <img align="center" src="https://socialify.git.ci/delta10/error-pages/image?description=1&font=Raleway&forks=1&issues=1&logo=https%3A%2F%2Fwww.delta10.nl%2F_next%2Fstatic%2Fmedia%2Flogo.e75acb6d.svg&owner=1&pulls=1&pattern=Solid&stargazers=1&theme=Light">
    </picture>
  </a>
</p>
...
$ docker run --rm \
  ...
  -p '8080:8080/tcp' ghcr.io/delta10/error-pages:3 serve \
...
COPY --chown=nginx \
     --from=ghcr.io/delta10/error-pages:3 \
...
services:
  ...
  error-pages:
    image: ghcr.io/delta10/error-pages:3 # using the latest tag is highly discouraged
    environment:
...
defaultBackend:
  enabled: true
  image:
    repository: ghcr.io/delta10/error-pages
...
errorPages:
  ...
  version: 3.3.1 # https://github.com/delta10/error-pages/releases
  themeName: shuffle
```
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: "{{ .appName }}"
          image: "ghcr.io/delta10/error-pages:{{ .version | default "latest" }}"
          env:
